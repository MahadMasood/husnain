...
export default function ElasticSlider({
  ...
  leftIcon = <>-</>,
  rightIcon = <>+</>,
  onChange, // 1. Accept onChange here
}) {
  return (
    <div className={`flex flex-col items-center justify-center gap-4 w-48 ${className}`}>
      <Slider
        ...
        leftIcon={leftIcon}
        rightIcon={rightIcon}
        onChange={onChange} // 2. Pass it down to the child
      />
    </div>
  );
}
...
function Slider({ 
  ...
  leftIcon, 
  rightIcon, 
  onChange // 3. Accept it in the child
}) {
  ...
  const handlePointerMove = e => {
    if (e.buttons > 0 && sliderRef.current) {
      ...
      // We check if onChange exists before calling it
      if (onChange) {
        onChange(newValue);
      }
